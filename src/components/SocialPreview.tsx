...
function TwitterPreview({
  ...
  const ringColor =
    remaining < 0 ? "#f4212e" : remaining < 20 ? "#ffd400" : "#1d9bf0";
    ...
    >
      <div style={{ display: "flex", gap: "12px" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            <span style={{ fontWeight: 800, fontSize: "1rem", color: textPri }}>
              Deadbeat® Official
            </span>
            ...
            <span style={{ fontSize: "0.88rem", color: textMuted }}>
              @deadbeat_studio · now
            </span>
          </div>
          ...
          <div
            style={{
              ...
              color: content ? textPri : textMuted,
          ...
          >
            ...
            <svg width="22" height="22" viewBox="0 0 22 22">
              ...
              />
              {remaining < 20 && (
                <text x="11" y="15" textAnchor="middle" fontSize="6.5" fontWeight="bold" fill={ringColor}>
                  {remaining}
                </text>
              )}
            </svg>
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: "0.92rem", color: textPri }}>
              deadbeat_studio
            </div>
      ...
      <div style={{ padding: "12px 16px 10px", background: card }}>
        ...
        <div style={{ fontSize: "0.88rem", fontWeight: 700, marginBottom: "5px", color: textPri }}>
          1,204 likes
        </div>
        ...
        <div style={{ fontSize: "0.73rem", color: dark ? "#555" : "#c7c7c7", marginTop: "4px", textTransform: "uppercase", letterSpacing: "0.05em" }}>
          1 hour ago
        </div>
      </div>
    </div>
...
function LinkedInPreview({
  ...
  return (
    <div style={{ background: bg, borderRadius: "8px", border: `1px solid ${border}`, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", color: textPri, overflow: "hidden" }}>
      ...
      {image && (
        <div style={{ overflow: "hidden", borderTop: `1px solid ${border}` }}>
          <img src={image} alt="post" style={{ width: "100%", maxHeight: "300px", objectFit: "contain", display: "block", background: dark ? "#111" : "#f7f7f7" }} />
        </div>
      )}
    ...
    <div style={{ background: bg, borderRadius: "8px", border: `1px solid ${border}`, fontFamily: "Helvetica, Arial, sans-serif", color: textPri, overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          <button style={{ background: btnBg, border: "none", borderRadius: "6px", padding: "7px 12px", fontWeight: 700, fontSize: "0.82rem", cursor: "pointer", color: textPri }}>
            + Follow
          </button>
      ...
      {image && (
        <div style={{ overflow: "hidden", borderTop: `1px solid ${border}`, borderBottom: `1px solid ${border}` }}>
          <img src={image} alt="post" style={{ width: "100%", maxHeight: "300px", objectFit: "contain", display: "block", background: dark ? "#18191a" : "#f7f7f7" }} />
        </div>
      )}
...
const SocialPreview: React.FC<SocialPreviewProps> = ({ platform, content, image }) => {
  const theme = useTheme();
  const dark = theme === "dark";

  switch (platform) {
    case "Twitter":   return <TwitterPreview   content={content} image={image} dark={dark} />;
    case "Instagram": return <InstagramPreview content={content} image={image} dark={dark} />;
    case "LinkedIn":  return <LinkedInPreview  content={content} image={image} dark={dark} />;
    case "Facebook":  return <FacebookPreview  content={content} image={image} dark={dark} />;
    default:          return <TwitterPreview   content={content} image={image} dark={dark} />;
  }
};
